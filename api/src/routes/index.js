...
const getApiRecipe = async () => {
  const apiUrl = await axios.get(
    `https://api.spoonacular.com/recipes/complexSearch?number=10&apiKey=${API_KEY}&addRecipeInformation=true`
  );
...
router.get("/recipe", async (req, res, next) => {
  let name = req.query.name;
  if (name) {
    let reqApi = await axios.get(
      `https://api.spoonacular.com/recipes/complexSearch?query=${name}&number=30&apiKey=${API_KEY}&addRecipeInformation=true`
    );
    let reqDb = await Recipes.findAll({
      //promesa
      where: {
        name: {
          [Op.iLike]: "%" + name + "%",
...
router.get("/recipeApi/id/:id/", async (req, res) => {
...
router.get("/recipeDB/id/:id/", async (req, res) => {
  let id = req.params.id;
